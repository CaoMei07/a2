...
model = AutoModelForCausalLM.from_pretrained(  
    r"/home/mint/Desktop/zipmoe/models/phi-mini-moe",  
    torch_dtype=torch.half,  
    device_map=None  # 避免自动设备映射  
)  
tokenizer = AutoTokenizer.from_pretrained(r"/home/mint/Desktop/zipmoe/models/phi-mini-moe")

# 设置专家级别的叶子节点  
set_z3_leaf_modules(model, ['PhimoeBlockSparseTop2MLP'])  
...
ds_config = {  
    "zero_optimization": {  
        ...
        "offload_param": {  
            "device": "nvme",  
            "nvme_path": "/home/mint/Desktop/zipmoe/nvme_offload",  
            "pin_memory": False,  
            "buffer_count": 64,  # 适中的缓冲区数量  
            "buffer_size": int(1.5e8),  # 稍小的缓冲区大小  
            "max_in_cpu": int(3e8)  # 增加 CPU 缓存  
        },  
        "zero_hpz_partition_size": 1,  
        "stage3_prefetch_bucket_size": int(8e7),  # 增加预取桶大小  
        "stage3_max_live_parameters": int(2e10),  # 增加活跃参数  
        "stage3_max_reuse_distance": int(3e11),   # 增加重用距离  
        "stage3_param_persistence_threshold": int(2e7),  # 降低持久化阈值  
        "overlap_comm": False  
    },  
    "fp16": {  
        "enabled": True  
...
ds_engine = deepspeed.init_inference(  
    ...
    moe={"enabled": True, "ep_size": 1, "moe_experts": [16]},  
...
if hasattr(ds_engine, 'parameter_offload'):    
    ...
    # 检查是否是您的专家感知调度器  
    if hasattr(coordinator, '_is_expert_module'):  
