...
# -----------------------zyr添加返回选择的专家id-------------------------
def get_selected_experts(logits: Tensor, k: int) -> Tensor:
    top_gate, top_idx = torch.topk(logits, k=k, dim=1)
    return top_idx
# --------------------------------------------------------
...
class TopKGate(Module):
    ...
    def __init__(self,
                 ...
                 output_mode: str = 'default') -> None:
        super().__init__()

        # print(f"[TOPK_GATE] TopKGate initialized with k={k}, num_experts={num_experts}") 
        #self.output_mode = output_mode  # 'default', 'logits_only', 'transformers_compatible'
    ...
    # ----------------------zyr外部设置------------------------
    def _set_expert_coordinator(self, expert_coordinator, layer_id):  
        self.expert_coordinator = expert_coordinator  
        self.layer_id = layer_id
        
        # 设置一个回调函数，在门控计算后调用  
        def update_expert_indices(logits, k):  
            try:  
                expert_indices = get_selected_experts(logits, k)  
                setattr(self.expert_coordinator, 'expert_indices', expert_indices.tolist())
                
                # 调用重置追踪状态的方法  
                if hasattr(self.expert_coordinator, 'reset_trace_state'):  
                    self.expert_coordinator.reset_trace_state()  
              
            except Exception as e:  
                # 回退到空列表，避免程序崩溃  
                setattr(self.expert_coordinator, 'expert_indices', [])  
        
        self.expert_coordinator.update_callback = update_expert_indices
    # -------------------zyr-------------------------
    #------------------------zyr 不同模型门控输出格式不同------------------
    def _detect_model_type(self):  
        ...
        # 检查当前模块中是否有 phi-moe 相关的类  
        for name, obj in sys.modules.items():  
            if 'phimoe' in name.lower() or 'phi_moe' in name.lower():  
                self.output_mode = 'transformers_compatible'  
                # print(f"[DETECT] Found phi-moe module: {name}")  
                return  
    ...
    def forward(self,
        ...
        # ------------------------zyr添加-----------------------------
        # 在门控计算完成后，如果有专家协调器，则调用回调  
        if hasattr(self, 'expert_coordinator') and hasattr(self.expert_coordinator, 'update_callback'):  
            #print(f"[GATE_CALLBACK] Calling update_callback with logits shape: {logits.shape}, k: {self.k}") 
            self.expert_coordinator.update_callback(logits, self.k)  
        else:
            pass
            #print(f"[GATE_CALLBACK] No expert_coordinator or update_callback found")
        # print(f"[Gate_output_mode] is {self.output_mode}")
        # 智能输出格式适配  
        if hasattr(self, 'output_mode'):  
            if self.output_mode == 'logits_only':  
                return logits  
            elif self.output_mode == 'transformers_compatible':  
                # 为 transformers 库格式返回  
                return logits  
            else:  
                return gate_output  
        else:  
...
class MOELayer(Base):
    ...
    def forward(self, *input: Tensor, **kwargs: Any) -> Tensor:
        ...
        if self.use_tutel:
            ...
            # ---------------------zyr---------------------
            # 在门控计算后添加  
            if hasattr(self, 'expert_coordinator'):  
                # 获取门控 logits  
                logits = self.gate.wg(reshaped_input)  # 重新计算或保存之前的 logits  
                self.expert_coordinator.log_expert_selection(  
                    layer_id=self.layer_id,  # 需要添加层ID跟踪  
                    logits=logits,  
                    k=self.gate.k  
                )
            # ---------------------zyr---------------------
