...
from ..constants import TORCH_DISTRIBUTED_DEFAULT_PORT, default_pg_timeout  # TODO：要写，必须，指定端口
from .constants import *  
from .reduce_op import *  
from deepspeed.comm.ccl import CCLBackend

from deepspeed.accelerator import get_accelerator      # 硬件加速器 （可选）
from deepspeed.utils.comms_logging import CommsLogger  # 性能监控 （可选）
from deepspeed.utils import timer, get_caller_func     # 性能监控 （可选）

from torch.distributed import GradBucket  # noqa: F401 # TODO：非必须，后续可删除 主要用于训练时的梯度同步
from datetime import timedelta    # 非必须，仅用于超时配置
...
def get_world_size(group=None) -> int:  
    ...
    # 检查是否在明确的单机推理模式下   
    try:  
        from deepspeed import _SINGLE_MACHINE_INFERENCE_MODE  
        if _SINGLE_MACHINE_INFERENCE_MODE:  
            return 1  
    except ImportError:  
        pass  # 变量未设置，继续正常逻辑  
