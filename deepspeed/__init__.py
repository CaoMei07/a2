...
# zyr添加的
_SINGLE_MACHINE_INFERENCE_MODE = False  # 是否是单机推理
...
from .runtime.engine import DeepSpeedOptimizerCallable, DeepSpeedSchedulerCallable
...
# from .runtime import zero, domino  # zyr改掉
...
def initialize(args=None,
    ...
    try:
        from .pipe import PipelineModule
        is_pipeline = isinstance(model, PipelineModule)
    except ImportError:
        is_pipeline = False
    if not is_pipeline:
    # ------------------------------------------------
...
def init_inference(model, config=None, **kwargs):
    ...
    """
    global _SINGLE_MACHINE_INFERENCE_MODE
    # -------------------原代码---------------------------
    log_dist("DeepSpeed info: version={}, git-hash={}, git-branch={}".format(__version__, __git_hash__,
                                                                             __git_branch__),
             ranks=[0])
    ...
    # ----------------zyr加入-----------------
    # 根据配置自动判断是否为单机模式
    tp_size = config_dict.get('tensor_parallel', {}).get('tp_size', 1)   
    ep_size = config_dict.get('moe', {}).get('ep_size', 1)  
   
    # 判断是否为单机推理模式  
    _SINGLE_MACHINE_INFERENCE_MODE = (tp_size <= 1 and ep_size <= 1)  
    # --------------------------------------------------------------------
