...
try:
    ...
    from deepspeed.utils import logger as accel_logger  # TODO：还没写
...
SUPPORTED_ACCELERATOR_LIST = ['cuda', 'cpu']  # 暂时只支持cpu和cuda
ds_accelerator = None
...
def is_current_accelerator_supported():  # 是否有加速器支持
    return get_accelerator().device_name() in SUPPORTED_ACCELERATOR_LIST


def get_accelerator():
    global ds_accelerator
    if ds_accelerator is not None:   # 采用单例模式，确保整个应用程序中只有一个加速器实例
    ...
    if "DS_ACCELERATOR" in os.environ.keys():
        ...
        # 下面注释掉的是deepspeed的原先代码，包含更多支持的依赖检查
    ...
    if accelerator_name is None:  # 自动检测机制
        ...
        if accelerator_name is None:
            try:
                ...
                pass
        if accelerator_name is None:  # cpu作为默认回退选项
    ...
    # 3. Set ds_accelerator accordingly  根据检测到的加速器类型创建相应的实例
