...
# from deepspeed.runtime.zero.config import DeepSpeedZeroConfig   # TODO:没写
# ---------zyr简化实现zero-------------------------
try:  
...
class DtypeEnum(Enum):
    ...
    @classmethod
    def from_str(cls, value: str): # 字符串到枚举值的转换
        for dtype in cls:
...
#     tp_size: int = 1  # 张量并行的设备数量
#     """ Number of devices to split the model across using tensor parallelism. """

#     tp_grain_size: int = 64  # MLP/lm_head的张量并行粒度大小
...
class DeepSpeedMoEConfig(DeepSpeedConfigModel):
    ...
    """
    # 全局专家数量 默认值1，类型list，别名num_experts
...
class BaseQuantConfig(DeepSpeedConfigModel):
    enabled: bool = True
    num_bits: int = 8   # 默认8位
    q_type: QuantTypeEnum = QuantTypeEnum.sym   # 默认对称
    q_groups: int = 1   # 默认1组
...
class InferenceCheckpointConfig(DeepSpeedConfigModel):
    ...
    save_mp_checkpoint_path: Optional[str] = None   # 模型并行检查点保存路径
...
class DeepSpeedInferenceConfig(DeepSpeedConfigModel):
    ...
    @field_validator("dtype", mode="before")   # 在字段解析前调用、
    def validate_dtype(cls, field_value, values): # 验证器 校验dtype字段的类型
    ...
    @field_validator("moe")
    def moe_backward_compat(cls, field_value, values): # 兼容旧的moe:true/false布尔字段写法
        if isinstance(field_value, bool):
            return DeepSpeedMoEConfig(moe=field_value)  # 包装成 DeepSpeedMoEConfig 类型
        return field_value

    @field_validator("use_triton")
    def has_triton(cls, field_value, values):  # 判断use_triton依赖项是否满足
