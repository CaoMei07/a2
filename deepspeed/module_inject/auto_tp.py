...
class AutoTP():
    ...
    def _replace(self, child, name, conv_linear_layer):
        print(f"[AutoTP] Processing module: {name}, type: {type(child)}")
        ...
        # 修改后 - 移除 block_sparse_moe.gate 条件  
        if "mlp.gate" == name or "q_a_proj" in name or "kv_a_proj_with_mqa" in name or (  
            ('mlp.shared_expert_gate' == name or 'mlp.gate' == name) and 'qwen2_moe' in str(type(self.module))):  
            return child
        # ------------------------------------------------
        # For Yuan model
