...
# from deepspeed import comm as dist                    # TODO：以下还没写
...
class DeepSpeedSelfAttention(nn.Module):
    ...
    def __init__(self, config, mp_group=None, q_scales=None, q_groups=1, merge_count=1):
        ...
        DeepSpeedSelfAttention.num_layers = DeepSpeedSelfAttention.num_layers + 1  # 每个实例递增
        device = get_accelerator().current_device_name()  #if config.bigscience_bloom else 'cpu'
        if self.config.set_empty_params:   # 用于初始化时占位，节省内存
            self.attn_qw = None
            self.attn_qb = None
            self.attn_kw = None
            self.attn_kb = None
        ...
        # Attention scaling 相关设置
        self.norm_factor = math.sqrt(self.config.hidden_size // self.config.heads)  # 根号下d_k
        if not config.use_mup:  # mup特殊缩放
            self.norm_factor = math.sqrt(self.norm_factor)

        if self.config.scale_attn_by_inverse_layer_idx is True:  # 层编号缩放
            self.norm_factor *= math.sqrt(self.config.layer_id + 1)
            # https://github.com/huggingface/transformers/blob/v4.24.0/src/transformers/models/gpt2/modeling_gpt2.py#L191
        # 核心操作函数注册
        self.qkv_func = QKVGemmOp(config)  # 执行Q/K/V权重矩阵乘
        self.score_context_func = SoftmaxContextOp(config)  # softmax+dropout
        self.linear_func = LinearOp(config)
        self.vector_matmul_func = VectorMatMulOp(config)  # 有KVcache的增量方式的向量矩阵乘
    ...
    def compute_attention(self, qkv_out, input_mask, layer_past, alibi, is_prompt, token_idx, position_ids):
        ...
        if no_masking: # 若没有提供 mask（比如推理阶段），就用一个空的Tensor占位
            input_mask = torch.empty(1)
        # 实际执行attention，调用的是一个fused kernel SoftmaxContextOp，可以是 Triton/CUDA 优化过的
        ...
        context_layer, key_layer, value_layer = attn_key_value  # 1就是最后的注意力输出
        return context_layer, key_layer, value_layer  # 23 用于保存 KV cache，供下次调用复用

    def _merge_qkv(self): # 将QKV的权重W_q,W_k,W_v（和偏置b_q,b_k,b_v）合并为一个大的QKV权重矩阵
        qvkw = DeepSpeedSelfAttention._qkv_buffers[0] # 将W_q,W_k,W_v按照顺序拼接在qvkw中，拼接维度是hidden_size
        ...
        if self.attn_qb is not None:  # 偏置部分
    ...
    def forward(self,
        ...
        if not self.config.pre_layer_norm:
            qkv_out = self.linear_func(input=input,
                                       ...
                                       num_layers=DeepSpeedSelfAttention.num_layers)
        else: # 调用 qkv_func，同时传入归一化参数
            qkv_out = self.qkv_func(input=input,
                                    weight=self._attn_qkvw,
                                    bias=self._attn_qkvb,
                                    gamma=norm_w,
                                    beta=norm_b)
        # 批次内 token 数大于 1 → 非增量 → 是 prompt
        ...
        output = self.vector_matmul_func(input=context_layer, weight=self.attn_ow) # Attention输出乘上输出权重矩阵
        inp_norm = qkv_out[-1]
...
class BloomSelfAttention(DeepSpeedSelfAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 额外初始化了SoftmaxOp操作符，因为 BLOOM 模型需要手动实现注意力计算而不是使用融合内核
