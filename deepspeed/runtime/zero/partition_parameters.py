...
from .linear import zero3_linear_wrap   # 线性层包装 训练时内存优化
...
from deepspeed.utils.debug import (debug_param2name_id_shape, debug_param2name_id_shape_device, debug_module2name,
                                    debug_param2name_id, debug_param2name_id_shape_status)    # 调试工具函数（zyr暂时注释掉）
...
from deepspeed.runtime.torch_autocast import sort_dtypes, get_autocast_dtype, has_autocast_dtype  # 量化（zyr暂时注释掉）
# 全局变量
partitioned_param_data_shape = [0]  # 用于跟踪分区参数的数据形状
zero_init_context = 0      # 跟踪 ZeRO 初始化上下文的嵌套深度
top_level_context = None   # 存储顶层初始化上下文的引用


class DeepSpeedTensorOverride(Enum): # 控制张量创建时的属性覆盖
    dtype = 1
    device = 2
...
# 根据参数的自动混合精度，设置确定all-gather使用的数据类型
...
class NoGatherHandle:  # 处理单个参数的“无收集”场景(当只有一个分区时)
    ...
    def __init__(self, param: Parameter) -> None:  
        ...
        if hasattr(param.ds_tensor, "ds_quant_scale"):  
            ...
            tensor_data = param.ds_tensor.data.to(device=get_accelerator().current_device_name(), non_blocking=True)  
            expected_numel = torch.tensor(param.ds_shape).prod().item()  
            actual_numel = tensor_data.numel()  
            
            if actual_numel != expected_numel:  
                print_rank_0(f"[SHAPE_MISMATCH] Param {param.ds_id}: expected {expected_numel}, got {actual_numel}")  
                # 使用零填充或截断来处理大小不匹配  
                if actual_numel < expected_numel:  
                    # 零填充  
                    padded_tensor = torch.zeros(expected_numel, dtype=tensor_data.dtype, device=tensor_data.device)  
                    padded_tensor[:actual_numel] = tensor_data.view(-1)  
                    param.data = padded_tensor.view(param.ds_shape)  
                else:  
                    # 截断  
                    param.data = tensor_data.view(-1)[:expected_numel].view(param.ds_shape)  
            else:  
                param.data = tensor_data.view(param.ds_shape)  
        
        self.__param = param

    def wait(self, **kwargs) -> None:
        if not get_accelerator().resolves_data_dependency(): # 加速器不能自动解析数据依赖
            get_accelerator().current_stream().synchronize()  # 手动同步当前流
        self.__param.ds_status = ZeroParamStatus.AVAILABLE


class NoGatherCoalescedHandle: # 处理多个参数的无收集场景
    ...
    def __init__(self, params: List[Parameter]) -> None:  
        ...
        for param in self.__params:  
            ...
            if actual_numel != expected_numel:  
                # 对于不完整的参数，需要特殊处理  
                print_rank_0(f"[INCOMPLETE_PARAM] Param {param.ds_id}: expected {expected_numel}, got {actual_numel}")  
                # 可能需要跳过这个参数或使用默认值  
                param.data = torch.zeros(param.ds_shape, dtype=param.dtype, device=get_accelerator().current_device_name())  
            else:  
                param.data = tensor_data.view(param.ds_shape)
...
def _init_external_params(module):
    if not hasattr(module, '_external_params'):
        ...
        def external_parameters(self):  # 返回外部参数
            return self._external_params.items()

        def all_parameters(self):   # 结合常规参数和外部参数
        ...
        .. code-block:: python
            ...
            class ModuleZ3(torch.nn.Module):
                ...
                def forward(self, input):
                    ...
                    return y
    """
    if not isinstance(parameter, torch.nn.Parameter):  # 参数验证
        raise RuntimeError('Parameter is not a torch.nn.Parameter')

    if not hasattr(module, '_external_params'): 
        _init_external_params(module)    # 初始化

    key = id(parameter)
    module._external_params[key] = parameter  # 注册
...
def get_all_subclasses(cls, include_root=True):  # 递归获取一个类的所有子类
    subclass_list = []
    ...
    if include_root:    # 可选包含根类
        ret.add(cls)
    return ret


@instrument_w_nvtx
def free_param(param: Parameter) -> None:  # 释放参数的底层存储
    ...
    if get_accelerator().on_accelerator(param.data):
        ...
        if not get_accelerator().is_synchronized_device():
            param.data.record_stream(get_accelerator().current_stream())  # 记录当前流以确保参数不会在仍被计算使用时被释放
    # param.data doesn't store anything meaningful in partitioned state
    param.data = torch.empty(0, dtype=param.dtype, device=param.device)  # 置空
    param.ds_status = ZeroParamStatus.NOT_AVAILABLE  # 更新状态

# 全局变量 -- 内存管理优化
reuse_buffers = False       # 控制是否重用缓冲区
temp_contiguous_tensor = None  # 临时连续张量的引用
empty_buffers = {}   # 空缓冲区字典
...
class InsertPostInitMethodToModuleSubClasses(object):
    ...
    def __init__(self, enabled=True, mem_efficient_linear=True, ds_config=None, dtype=None):
        self.mem_efficient_linear = mem_efficient_linear  # 是否使用内存高效的线性层实现
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if zero_init_context == 0:
            self.unpatch_init_and_builtins()    # 清理
            global top_level_context
            ...
            if dist.get_rank() == 0:
                ...
                logger.info(
                    f"finished initializing model - num_params = {num_params}, num_elems = {billion_elems:.2f}B")  # Billion(十亿个元素)
    ...
    def _set_dtype(self, ds_config, dtype):
        if ds_config is not None and dtype is None:
            if ds_config.bfloat16_config.enabled and ds_config.float16_config.enabled:
                raise RuntimeError("bfloat16 and fp16 cannot be enabled at once") # 冲突检查
    ...
    def patch_init_and_builtins(self):
        ...
        def hook_for_skip_init(module):
            ...
            def post_wrapper_to_empty(f):  # 恢复
            ...
            for subclass in get_all_subclasses(torch.nn.modules.module.Module):  # 钩子安装
                _enable_class_apply(subclass)

            # add a restore hook when exiting skip_init
            module.to_empty = post_wrapper_to_empty(module.to_empty)  # 为.to_empty恢复钩子
        ...
        def partition_after(f):
            ...
            def wrapper(module, *args, **kwargs):
                ...
                if not hasattr(module, "_ds_child_entered"): # 子模块检测
    ...
    def unpatch_init_and_builtins(self):  # 恢复所有被修改的pytorch原始行为
        if self.patched:
            ...
            for subclass in get_all_subclasses(torch.nn.modules.module.Module):
                _disable_class(subclass)  # 遍历所有子类，恢复原始初始化
    ...
    def _add_tensor_creation_wrappers(self):  # 包装 PyTorch 的张量创建函数
    ...
    def _remove_tensor_creation_wrappers(self):  # 恢复原始的张量创建函数
...
def shutdown_init_context():  # 初始化期间，临时移除包装器但保持上下文
    """
...
def restore_init_context():  # 初始化完成后恢复包装器
    """
...
class AllGatherHandle:  # 单个参数 all-gather 操作的句柄，用于管理异步参数收集
    ...
    def wait(self, handle_dependency=True) -> None:  # 处理参数收集的完成
...
class AllGatherCoalescedHandle:
    ...
    def wait(self, handle_dependency=True) -> None:
        ...
        for param in self.params:
            assert param.ds_status == ZeroParamStatus.INFLIGHT, f"expected param {param.ds_summary()} to be inflight"
            partitions: List[Tensor] = []
            ds_tensor_numel = param.ds_tensor.ds_numel  # 每个 rank 在这个参数上负责的元素数量
            if self.use_secondary_tensor:
                ds_tensor_numel *= param.ds_secondary_tensor_num_of_groups
            for rank in range(self.world_size):   # 分区收集
                param_start = rank * ds_tensor_numel
                if param_start < param.ds_numel:  # 提取出这个参数对应的分片 min()处理最后一个shard可能不满的问题
            ...
            param.data = instrument_w_nvtx(torch.cat)(partitions).view(param.ds_shape).to(param.ds_tensor.dtype) # 拼接并还原
            param.ds_status = ZeroParamStatus.AVAILABLE
...
class MultipleAllGatherHandles:  # 管理多个AllGatherCoalescedHandle的批量操作
...
class AllReduceCoalescedHandle:  #  all-gather 的替代方案
...
class Init(InsertPostInitMethodToModuleSubClasses):
    ...
    def __init__(self,
        ...
        """
        if config is not None:  # config兼容性
        ...
        if not dist.is_initialized():  # 分布式初始化（对参数分区是必须的）
            init_distributed()
            assert dist.is_initialized(), "Parameters cannot be scattered without initializing deepspeed.comm"

        if data_parallel_group is None:  # 设置数据并行进程组
        ...
        get_accelerator().set_device(self.local_device)   # 设置本地设备
        # 量化配置
        ...
        if self.remote_device == OffloadDeviceEnum.nvme:   # 配置NVMe交换器
...
# -------------------------------------------------------
    def _validate_param_shape_consistency(self, param, actual_data_size):  
        """验证参数形状一致性"""  
        expected_size = param.ds_shape.numel() if hasattr(param.ds_shape, 'numel') else torch.tensor(param.ds_shape).prod().item()  
        if actual_data_size != expected_size:  
            print_rank_0(f"[SHAPE_MISMATCH] Param {param.ds_id}: expected {expected_size}, got {actual_data_size}")  
            return False  
        return True  
    ...
    def _ensure_availability_of_partitioned_params(self, params):  
        ...
        # 处理正在换入的参数  
            ...
            # 添加形状一致性验证和处理  
            for param in swap_in_flight:  
                if hasattr(param.ds_tensor, 'data'):  
                    actual_size = param.ds_tensor.data.numel()  
                    expected_size = param.ds_shape.numel() if hasattr(param.ds_shape, 'numel') else torch.tensor(param.ds_shape).prod().item()  
                    
                    if not self._validate_param_shape_consistency(param, actual_size):  
                        # 处理形状不匹配的情况  
                        print_rank_0(f"[SHARED_BUFFER_RECOVERY] Attempting to recover param {param.ds_id}")  
                        
                        if actual_size < expected_size:  
                            # 如果实际大小小于期望大小，用零填充  
                            print_rank_0(f"[ZERO_PADDING] Padding param {param.ds_id} from {actual_size} to {expected_size}")  
                            
                            # 创建正确大小的张量并用零填充  
                            correct_tensor = torch.zeros(expected_size,   
                                                    dtype=param.ds_tensor.data.dtype,   
                                                    device=param.ds_tensor.data.device)  
                            
                            # 复制实际可用的数据  
                            copy_size = min(actual_size, expected_size)  
                            correct_tensor[:copy_size] = param.ds_tensor.data.view(-1)[:copy_size]  
                            
                            # 更新参数张量  
                            param.ds_tensor.data = correct_tensor  
                            param.ds_tensor.status = PartitionedParamStatus.AVAILABLE  
                            
                        elif actual_size > expected_size:  
                            # 如果实际大小大于期望大小，截断  
                            print_rank_0(f"[TRUNCATION] Truncating param {param.ds_id} from {actual_size} to {expected_size}")  
                            param.ds_tensor.data = param.ds_tensor.data.view(-1)[:expected_size]  
                            param.ds_tensor.status = PartitionedParamStatus.AVAILABLE  
                    else:  
                        # 形状匹配，正常处理  
                        param.ds_tensor.status = PartitionedParamStatus.AVAILABLE
