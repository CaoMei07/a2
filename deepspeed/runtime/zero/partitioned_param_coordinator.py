...
ENABLE_PROFILER = False    # 全局开关，控制是否开启性能分析功能


def debug_rank0(message: str) -> None:   # 确保调试信息只在rank 0进程上输出
    if dist.get_rank() == 0:
        logger.debug(message)


@instrument_w_nvtx
def get_all_parameters(sub_module, recurse=False):  # 参数获取函数
    return itertools.chain(sub_module.named_parameters(recurse=recurse), sub_module.ds_external_parameters())


@compiler.enable(min_version="2.7.0")  
def iter_params(module: Module, recurse=False) -> Iterable[Parameter]:
    return map(lambda pair: pair[1], get_all_parameters(module, recurse)) # 只返回参数对象而不包含名称
...
class PartitionedParameterCoordinator:
    ...
    @dataclass
    class __ParamInTrace:  # 在执行轨迹中记录参数信息
        param: Parameter
    ...
    def __init__(
        ...
        self.__step_id: int = 0  # 步骤ID，跟踪子模块调用次数
        ...
        self.__max_ongoing_fetch_events: int = 2  #  最大并发获取事件数量（默认为2）
    ...
    def _clear_trace_structures(self) -> None:  # 清空跟踪相关的数据结构
    ...
    @compiler.enable(min_version="2.7.0")
    def record_module(self, sub_module: Module) -> None:  # 模块记录
    ...
    def record_parameters(self, sub_module: Module) -> None:  # 将子模型的参数添加到执行轨迹中
        if is_compiling():
            return
        """adds sub module to trace"""
        if not self.is_record_trace():
    ...
    def construct_parameter_trace_from_module_trace(self):
        ...
        for sub_module in self.__submodule_order:
            self.record_parameters(sub_module)  # 模块级别的执行轨迹转换为更细粒度的参数级别轨迹
    ...
    def reset_step(self) -> None:
        ...
        self._clean_inflight_param_registry()  # 等待并释放所有正在进行的参数
        ...
        if not self.is_complete_trace():  # not self.trace_complete:
            ...
            if self.is_record_trace():
                ...
                assert_ints_same_as_other_ranks([p.param.ds_id for p in self.__param_order])  # 确保所有进程的轨迹一致
    ...
    # 调试输出
    def _dump_params(self, tag, sub_module, params, step_id=None): # 输出参数的名称ID形状
        if step_id is None:
    ...
    def _dump_param_ids(self, tag, mod_id, p_ids, step_id=None):  # 输出模块和参数ID的简化信息
        if step_id is None:
            step_id = self.__step_id
    ...
    def fetch_sub_module(self, current_submodule: Module, forward: bool) -> None: # 负责为子模块获取所需的参数
        ...
        """
        if logger.isEnabledFor(logging.DEBUG):  # 调试信息输出
            debug_rank0(
        ...
        if fetch_numel > 0:
            ...
            self.__profiler.start_event(event_name)  # 启动性能分析事件
            ...
            # # -----------zyr记录时间----------------
            # import time
            # if not hasattr(self, 'expert_truefetch_latency'):    
            #     self.expert_truefetch_latency = []
            # fetch_start_time = time.perf_counter()
            self.__all_gather_params(params_to_fetch, forward)   # 异步获取
            # fetch_end_time = time.perf_counter()
            # fetch_duration = fetch_end_time - fetch_start_time
            # # 只为专家模块记录时间  
            # if current_submodule.__class__.__name__ == 'DeepseekMLP':  
            #     self.expert_truefetch_latency.append(fetch_duration)
            # # ------------------------------------
        ...
        for param in params_to_fetch:
            param.ds_active_sub_modules.add(current_submodule.ds_id) # 将当前子模块ID添加到参数的活跃子模块集合中
            if logger.isEnabledFor(logging.DEBUG):
                debug_rank0(f"-wait: {param.ds_summary()}")
            if param in self.__inflight_param_registry:
                wait_numel += param.partition_numel()
                with get_accelerator().stream(self.__allgather_stream):
                    while self.__ongoing_fetch_events and self.__ongoing_fetch_events[0].query():  # query() 方法检查事件是否已完成
                        self.__ongoing_fetch_events.popleft()  # 清理已完成的获取事件
                    if len(self.__ongoing_fetch_events) > self.__max_ongoing_fetch_events:
                        self.__ongoing_fetch_events.popleft().synchronize() # 限制并发获取事件数量
                    # 参数句柄等待 -- 从注册表中移除参数并等待其 all-gather 操作完成
        ...
        if not get_accelerator().resolves_data_dependency(): # 若加速器不能自动解析数据依赖，手动等待 all-gather 流完成
            get_accelerator().current_stream().wait_stream(self.__allgather_stream)
        if fast_fetch:  # 快速获取模式下，释放合并句柄的缓冲区
        ...
        if self.is_complete_trace():
            ...
            while self.__param_queue and len(discarded_from_prefetch_queue) < len(params_not_already_fetched):
                ...
                discarded_from_prefetch_queue.add(param_in_trace.param) # 从预取队列中移除当前参数
            # 跟踪一致性验证
            ...
            if self.__prefetch_bucket_sz > 0:
                max_params_to_prefetch = min(self.__max_n_available_params - self.__n_available_params,
                                             self.__prefetch_bucket_sz)  # 计算可以预取的最大参数数量
                ...
                while self.__param_queue and numel_prefetching < max_params_to_prefetch:
                    ...
                    if do_prefetch: # 只有状态为 NOT_AVAILABLE 且不在当前预取集合中的参数
                        params_to_prefetch.add(param_in_trace.param)  # 加入预取集合
                        numel_prefetching += param_in_trace.param.ds_numel

                if numel_prefetching > 0:   # 执行预取操作
                    ...
                    self.__all_gather_params(params_to_prefetch, forward)  # 执行实际的参数收集操作
    ...
    def release_sub_module(self, submodule: Module, forward=False) -> None:
        ...
        for param in iter_params(submodule, recurse=z3_leaf_module(submodule)):
            param.ds_active_sub_modules.discard(submodule.ds_id)  # 从参数的活跃模块集合中移除当前模块
            if param.ds_id in params_to_release and not param.is_external_param:
                self.__release_param(param, free_data)  # 释放
            if not free_data:
                if param.ds_id in params_to_release and not param.is_external_param:
    ...
    def release_and_reset_all(self, module: Module) -> None:
        ...
        for param in iter_params(module, recurse=True):
            if param in self.__inflight_param_registry:
                self.__inflight_param_registry.pop(param).wait()  # 清理正在进行的操作
        ...
        for param in iter_params(module, recurse=True):
            if param.ds_status != ZeroParamStatus.NOT_AVAILABLE: # 验证所有参数都已正确释放到 NOT_AVAILABLE 状态
    ...
    def __release_param(self, param: Parameter, free_data: bool = True) -> None:
        if param.ds_status == ZeroParamStatus.AVAILABLE and not param.ds_active_sub_modules:
            ...
            param.partition(free_data=free_data)  # 将参数重新分区
    ...
    def __params_to_release(self, submodule_to_release: Module, step_id: int) -> Set[int]:
        ...
        for param in iter_params(submodule_to_release, recurse=z3_leaf_module(submodule_to_release)):
            if self.__most_recent_step_id_param_fetched_for[param] > step_id:
                params_to_release.discard(param.ds_id)   # 预取跳过检测
        ...
        for module in self.__submodule_order[step_id:]:   
            if params_traversed >= self.__max_reuse_dist_in_numel:   # 重用距离检查
                break
    ...
    def __prefetch_nvme_param_partitions(self) -> None:
        ...
        for param_in_trace in self.__param_queue:
            ...
            if param.nvme_swapper is None:   # 跳过非 NVMe 参数
                continue
        ...
        if swap_in_params:  # 异步操作将它们从 NVMe 交换到内存中
