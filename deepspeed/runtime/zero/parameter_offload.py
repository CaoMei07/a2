...
from deepspeed.runtime.zero.expert_aware_coordinator import ExpertAwareParameterCoordinator  
...
# 替换PyTorch模块中存储参数的_parameters属性，以便在访问参数时能够检测和处理外部的ZeRO参数
    ...
    def __getitem__(self, key):
        ...
        if hasattr(param, "ds_status") and param.ds_status == ZeroParamStatus.NOT_AVAILABLE:
            if self._parent_module._parameters._in_forward:
                register_external_parameter(FWD_MODULE_STACK[-1], param) # 将参数注册为外部参数
                param.all_gather()   # 将该参数从CPU/NVMe加载到GPU
                print_rank_0(f'Registering external parameter from getter {key} ds_id = {param.ds_id}', force=False)

        return param

# 遍历子模块，并将其_parameters属性替换为ZeROOrderedDict或其他指定的cls实例
...
class DeepSpeedZeRoOffload(object):
    ...
    def __init__(
        ...
        prefetch_bucket_size=50000000,   # 预取桶的大小，控制一次预取多少参数
        max_reuse_distance=1000000000,   # 参数重用的最大距离，超过此距离的参数会被释放
        max_live_parameters=1000000000,   # 同时在 GPU 上保持活跃的最大参数数量
        param_persistence_threshold=100000,   # 参数持久化阈值，小于此值的参数会保持在 GPU 上
        model_persistence_threshold=sys.maxsize,   # 模型级别的持久化阈值
        dp_process_group=None,       # 模型级别的持久化阈值
        offload_param_config=None,   # 参数卸载配置，指定卸载到 CPU 还是 NVMe
        #mpu=None,                    # 模型并行单元
        zero_param_parallel_group=None,   # ZeRO 参数并行组
        zero_quantized_weights=False,     # 是否对权重进行量化
        zero_quantized_nontrainable_weights=False,    # 是否对非训练权重进行量化
        zero_module_granularity_threshold=0,     # 模块粒度阈值
        log_trace_cache_warnings=False,   # 是否记录跟踪缓存警告
    ):
        # 内存使用监控
        ...
        self._convert_to_zero_parameters(ds_config, module)  
        # 将普通参数转换为 ZeRO 参数
        ...
        self.param_coordinator = ExpertAwareParameterCoordinator( 
            prefetch_bucket_sz=self._prefetch_bucket_sz,  
            max_reuse_distance_in_numel=self._max_reuse_distance_in_numel,  
            max_available_parameters_in_numel=self._max_available_parameters_in_numel,  
            allgather_stream=self.__allgather_stream,  
        ...
        # if has_moe_layers(self.module)[0]:
        #     self.param_coordinator = ExpertAwareParameterCoordinator(
        #         model=self.module,  
        #         config={ 
        #             'prefetch_bucket_sz':self._prefetch_bucket_sz,
...
# ----------------------------zyr----------------------------------------------     
        ...
        self.setup_zero_stage3_hooks()  # 在模块上注册实际的钩子
        print_rank_0(
    ...
    @instrument_w_nvtx
    def partition_all_parameters(self):  # 强制分区所有未分区的参数
    ...
    def empty_partition_cache(self):   # 清空分区缓存
        self.partition_all_parameters()

    def _convert_to_zero_parameters(self, ds_config, module):
        non_zero_params = [p for p in module.parameters() if not is_zero_param(p)]
        if non_zero_params: 
            zero_params = [p for p in module.parameters() if is_zero_param(p)]
            if zero_params:  # 如果已经存在ZeRO参数，使用现有的第一个ZeRO参数来转换其他参数
                ...
                # Init类是一个上下文管理器，用于大规模模型构建，自动分区参数并转换为半精度
                Init(module=module,
    ...
    def setup_zero_stage3_hooks(self):
        self.hierarchy = 0   # 初始化层次结构

        #reset step if in inference mode
        @instrument_w_nvtx
        def _start_of_forward_hook(module, *args):   # 前向开始钩子

            self.get_param_coordinator().reset_step()   # 在前向传播开始时重置参数协调器的步骤计数

        self.fwd_pre_hook = self.module.register_forward_pre_hook(_start_of_forward_hook)

        #likely one of them should be enough but just to be safe
        self._register_deepspeed_module(self.module)   # 为模块及其子模块注册详细的钩子系统

        # Add top module to stack trace
        global FWD_MODULE_STACK
        FWD_MODULE_STACK.append(self.module)    # 跟踪前向传播的模块调用栈
    # 标记哪些参数应该持久保存在 GPU 内存中，避免频繁的卸载和加载
    def mark_persistent_parameters(self, param_threshold, model_threshold):
        ...
        for name, param in self.module.named_parameters(recurse=True):
            if param.ds_numel + total_persistent_parameters > model_threshold:  
                continue   # # 添加当前参数会超过 model_threshold，则跳过

            if param.ds_numel <= param_threshold:   # 参数大小小于等于param_threshold，则标记为持久参数
                params_count += 1
    ...
    def _register_deepspeed_module(self, module, count=[0]):
        ...
        if z3_leaf_module(module):
            for param in module.parameters():
                param.ds_z3_leaf_module = module  # 为模型的每个模块注册钩子以管理参数的生命周期
        else:
        ...
        def _pre_forward_module_hook(module, *args):   # 前向预钩子
            self.pre_sub_module_forward_function(module)

        @instrument_w_nvtx
        def _post_forward_module_hook(module, input, output):  # 在模块前向传播完成后被调用
            # 模块栈管理
            global FWD_MODULE_STACK  # 全局前向模块栈中移除当前模块，这个栈用于跟踪模块的调用层次
            FWD_MODULE_STACK.pop()   
            if output is None:  # 输出标准化为列表格式
                output = []
        ...
        # 前向后钩子（在前向传播完成后注册的钩子），用于在反向传播开始前准备参数
        ...
        def _alternate_post_backward_module_hook(module, inputs):  # 替代的后向传播钩子实现
            module.ds_grads_remaining = 0   # 引用计数

            #print(f"Before Forward {module.__class__.__name__}")

            def _run_after_backward_hook(*unused): # 每次调用时减少计数器，当计数器归零时调用后向传播清理函数
            ...
            def _run_before_forward_function(input): # 在前向传播时为需要梯度的张量增加计数
        ...
        @torch.compiler.disable
        def _post_backward_module_hook(module, inputs):   # 标准的后向传播钩子
        ...
        if not hasattr(module, "pre_bwd_fn"):
            ...
            class PreBackwardFunctionForModule(torch.autograd.Function): # 在反向传播前执行参数预取
    ...
    @torch.no_grad()
    def pre_sub_module_forward_function(self, sub_module): # 子模块前向传播开始前被调用
        see_memory_usage(f"Before sub module function {sub_module.__class__.__name__}", force=False)  # 记录子模块执行前的内存使用情况
        # 模块栈管理
        global FWD_MODULE_STACK
        FWD_MODULE_STACK.append(sub_module)

        param_coordinator = self.get_param_coordinator()
        param_coordinator.trace_prologue(sub_module)  # 检查当前模块是否符合预期的执行轨迹
        if param_coordinator.is_record_trace():  # 将模块添加到执行轨迹中
            param_coordinator.record_module(sub_module)  # 将模块添加到执行轨迹中
        param_coordinator.fetch_sub_module(sub_module, forward=True)  # 预取该模块所需的参数

        see_memory_usage(f"Before sub module function {sub_module.__class__.__name__} after fetch", force=False)

    @torch.no_grad()
    def post_sub_module_forward_function(self, sub_module):  # 在子模块前向传播完成后被调用
        see_memory_usage(
            f"After sub module function {sub_module.__class__.__name__} {sub_module.ds_id} before release",
            force=False)

        param_coordinator = self.get_param_coordinator()
        param_coordinator.release_sub_module(sub_module, forward=True)  # 释放该模块的参数
    ...
    @torch.no_grad()
    def pre_sub_module_backward_function(self, sub_module):  # 子模块反向传播开始前被调用
        # assert sub_module.training, "backward pass is invalid for module in evaluation mode"
        param_coordinator = self.get_param_coordinator()
        param_coordinator.trace_prologue(sub_module)   # 检查反向传播的执行轨迹
        if param_coordinator.is_record_trace():
            param_coordinator.record_module(sub_module)  # 记录反向传播的模块顺序
        param_coordinator.fetch_sub_module(sub_module, forward=False)  # 预取

    @torch.no_grad()
    def post_sub_module_backward_function(self, sub_module):  # 子模块反向传播完成后被调用，负责清理工作
        # assert sub_module.training, "backward pass is invalid for module in evaluation mode"
        see_memory_usage(
            f"After sub module backward function {sub_module.__class__.__name__} {sub_module.ds_id} before release",
            force=False)

        self.get_param_coordinator().release_sub_module(sub_module, forward=False)  # 释放

        see_memory_usage(
            f"After sub module backward function {sub_module.__class__.__name__} {sub_module.ds_id} after release",
            force=False)
    # 模块粒度优化的主入口函数，根据给定的阈值决定哪些模块应该被设置为叶子模块
    def _set_z3_leaf_modules_by_threshold(self, module, zero_module_granularity_threshold):

        self._get_granularity_recursively(module)  # 计算所有模块的粒度
    ...
    def _get_granularity_recursively(self, module):
        ...
        if not list(module.parameters()): # 无参数模块：如 GELU 等激活函数，设置粒度为 sys.maxsize
        ...
        if not any(module.children()): # PyTorch 叶子模块：没有子模块的模块，设置粒度为 sys.maxsize
            # torch leaf module
            module.ds_model_granularity = sys.maxsize
            return 1, num_params
        ...
        # 容器模块：如 ModuleList，设置粒度为 sys.maxsize 以避免影响子模块的钩子设置
        ...
        num_layers += 1   # 超大模块：参数数量超过 Z3_MAX_LEAF_SIZE（10亿）的模块，设置粒度为 sys.maxsize
