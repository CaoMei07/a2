...
class AsyncPartitionedParameterSwapper(object):
    ...
    def __init__(self, ds_config, model_dtype):
        ...
        self.buffer_id_to_param_ids = {}   # 记录每个缓冲区包含的参数列表
        self.param_id_to_buffer_offset = {}    # 记录参数在共享缓冲区中的偏移量
        
        # 记录每个缓冲区的使用情况  
        self.buffer_id_to_usage = {}  # {buffer_id: {'used_bytes': int, 'allocations': [(offset, size, param_id)]}}
        # --------------------------------------------------------------
    ...
    def _get_swap_paths(self, params, must_exist=False):    
        ...
        for param in params:    
            ...
            if param_id in self.id_to_path.keys():    
                ...
                # 严格验证文件大小并修复  
                if must_exist and os.path.exists(param_path):    
                    expected_size = self._io_aligned_numel(self.param_id_to_numel[param_id]) * self.swap_element_size    
                    actual_size = os.path.getsize(param_path)    
                    if actual_size < expected_size:    
                        print_rank_0(f"Error: File {param_path} size {actual_size} < expected {expected_size}, recreating file")  
                        # 删除损坏的文件，强制重新创建  
                        os.remove(param_path)  
                        del self.id_to_path[param_id]  
                        param_path = os.path.join(self.swap_folder, f'{param_id}_param.tensor.swp')  
                        self.id_to_path[param_id] = param_path  
            else:    
                assert not must_exist, f"Path for param id {param_id} does not exist"    
    ...
    def _allocate_and_return_buffers_for_swap_in(self, params):  
        ...
        # 按参数大小排序，优化分配策略  
        sorted_params = sorted(params, key=lambda p: self.param_id_to_numel[p.ds_id], reverse=True)  
        
        for param in sorted_params:  
            param_id = param.ds_id  
            assert param_id in self.param_id_to_numel.keys(), f"Number of elements in param {param_id} is unknown"  
            assert param_id not in self.param_id_to_buffer_id.keys(), f"param {param_id} already assigned"  
            
            param_numel = self.param_id_to_numel[param_id]  
            aligned_numel = self._io_aligned_numel(param_numel)  
            
            # 尝试在现有缓冲区中找到空间  
            allocated_buffer_id = self._find_or_allocate_buffer(param_id, aligned_numel)  
            
            if allocated_buffer_id is None:  
                # 分配新的缓冲区  
                buffer_id = self.available_buffer_ids.pop()  
                self._initialize_new_buffer(buffer_id, param_id, aligned_numel)  
                allocated_buffer_id = buffer_id  
            
            # 创建缓冲区视图  
            offset = self.param_id_to_buffer_offset[param_id]  
            buffer_start = int(allocated_buffer_id * self.aligned_elements_per_buffer + offset)  
            
            swap_buffer = self.buffers.narrow(0, buffer_start, aligned_numel)  
            compute_buffer = swap_buffer.narrow(0, 0, param_numel)  
            
            self.param_id_to_swap_buffer[param_id] = swap_buffer  
            compute_buffers.append(compute_buffer)  
            swap_buffers.append(swap_buffer)  
            
            print_rank_0(f"param {param_id} allocated in buffer {allocated_buffer_id} at offset {offset}")  
        
        return compute_buffers, swap_buffers  
    
    def _find_or_allocate_buffer(self, param_id, aligned_numel):    
        """在现有缓冲区中寻找足够空间，如果找不到返回None - 修复偏移量"""    
        assert (aligned_numel <= self.aligned_elements_per_buffer,     
            f"param {param_id} size {aligned_numel} exceeds buffer capacity {self.aligned_elements_per_buffer}")    
        
        # 查找有足够连续空间的缓冲区  
        for buffer_id, usage in self.buffer_id_to_usage.items():    
            available_space = self.aligned_elements_per_buffer - usage['used_bytes']    
            if available_space >= aligned_numel:    
                # 使用已使用的字节数作为偏移量，确保连续分配  
                offset = usage['used_bytes']    
                
                # 验证偏移量计算正确性  
                if offset + aligned_numel <= self.aligned_elements_per_buffer:  
                    self._allocate_in_existing_buffer(buffer_id, param_id, aligned_numel, offset)    
                    return buffer_id    
        return None
    
    def _initialize_new_buffer(self, buffer_id, param_id, aligned_numel):  
        """初始化新缓冲区的使用情况"""  
        self.param_id_to_buffer_id[param_id] = buffer_id  
        self.param_id_to_buffer_offset[param_id] = 0  
        
        # 初始化缓冲区使用情况  
        self.buffer_id_to_param_ids[buffer_id] = [param_id]  
        self.buffer_id_to_usage[buffer_id] = {  
            'used_bytes': aligned_numel,  
            'allocations': [(0, aligned_numel, param_id)]  
        }  
    
    def _allocate_in_existing_buffer(self, buffer_id, param_id, aligned_numel, offset):    
        """在现有缓冲区中分配空间 - 修复偏移量计算"""    
        self.param_id_to_buffer_id[param_id] = buffer_id    
        self.param_id_to_buffer_offset[param_id] = offset    
        
        # 验证偏移量不会超出缓冲区边界  
        if offset + aligned_numel > self.aligned_elements_per_buffer:  
            raise RuntimeError(f"Buffer allocation would exceed buffer capacity: {offset} + {aligned_numel} > {self.aligned_elements_per_buffer}")  
        
        # 更新缓冲区使用情况    
        self.buffer_id_to_param_ids[buffer_id].append(param_id)    
        self.buffer_id_to_usage[buffer_id]['used_bytes'] += aligned_numel    
        self.buffer_id_to_usage[buffer_id]['allocations'].append((offset, aligned_numel, param_id))
    ...
    def synchronize_reads(self):  
        ...
        for param, swap_in_buffer in zip(self.inflight_params, self.inflight_swap_in_buffers):  
            ...
            # 对于共享缓冲区，需要检查实际可用的缓冲区大小  
            expected_numel = self.param_id_to_numel[param_id]  
            actual_buffer_numel = swap_in_buffer.numel()  
            
            # 使用实际可用的大小，而不是期望的大小  
            actual_numel = min(expected_numel, actual_buffer_numel)  
            
            if actual_numel != expected_numel:  
                print_rank_0(f"[SHARED_BUFFER_SYNC] Param {param_id}: expected {expected_numel}, got {actual_numel}")  
            
            compute_buffer = swap_in_buffer.narrow(0, 0, actual_numel)  
    ...
    def remove_partition_and_release_buffers(self, params):  
        for param in params:  
            ...
            if param_id in self.param_id_to_buffer_id.keys():  
                ...
                # 从缓冲区的参数列表中移除此参数  
                if buffer_id in self.buffer_id_to_param_ids:  
                    self.buffer_id_to_param_ids[buffer_id].remove(param_id)  
                    
                    # 更新缓冲区使用情况  
                    if buffer_id in self.buffer_id_to_usage:  
                        # 移除此参数的分配记录  
                        allocations = self.buffer_id_to_usage[buffer_id]['allocations']  
                        param_allocation = next((alloc for alloc in allocations if alloc[2] == param_id), None)  
                        if param_allocation:  
                            allocations.remove(param_allocation)  
                            self.buffer_id_to_usage[buffer_id]['used_bytes'] -= param_allocation[1]  
                    
                    # 如果缓冲区为空，释放整个缓冲区  
                    if len(self.buffer_id_to_param_ids[buffer_id]) == 0:  
                        self.available_buffer_ids.append(buffer_id)  
                        del self.buffer_id_to_param_ids[buffer_id]  
                        del self.buffer_id_to_usage[buffer_id]  
                        print_rank_0(f"buffer {buffer_id} completely released")  
                
                # 清理参数映射  
                del self.param_id_to_buffer_id[param_id]  
                del self.param_id_to_swap_buffer[param_id]  
                del self.param_id_to_buffer_offset[param_id]  
                
                print_rank_0(f"param {param_id} releases buffer id {buffer_id}")  
    ...
    def _swap_out(self, params, async_op=True):  
        ...
        # 在写入前验证缓冲区大小  
        for i, (param, buffer) in enumerate(zip(params, swap_out_params)):  
            param_id = param.ds_id  
            expected_size = self._io_aligned_numel(self.param_id_to_numel[param_id]) * self.swap_element_size  
            buffer_size = buffer.numel() * self.swap_element_size  
            
            if buffer_size != expected_size:  
                print_rank_0(f"Warning: Buffer size mismatch for param {param_id}: {buffer_size} vs expected {expected_size}")  
    ...
    def swap_in(self, params, async_op=True, swap_in_buffers=None):
        ...
        if swap_in_buffers is None:
            ...
            # -----------------------从检查缓冲区数量改为检查总容量-------------------
            # 计算所需的总容量而不是缓冲区数量  
            total_required_capacity = sum([self._io_aligned_numel(self.param_id_to_numel[p.ds_id]) for p in params])  
            total_available_capacity = len(self.available_buffer_ids) * self.aligned_elements_per_buffer  
            for buffer_id in self.buffer_id_to_usage.keys():  
                total_available_capacity += (self.aligned_elements_per_buffer - self.buffer_id_to_usage[buffer_id]['used_bytes'])  
            
            assert (total_required_capacity <= total_available_capacity,  
                f"Not enough buffer capacity {total_available_capacity} for swapping {total_required_capacity} elements")
    ...
    def swap_into_buffer(self, param, dest_buffer):  
        ...
        if require_swap_buffer:  
            # 使用新的容量检查逻辑而不是简单的缓冲区数量检查  
            param_numel = self.param_id_to_numel.get(param.ds_id, dest_buffer.numel())  
            aligned_numel = self._io_aligned_numel(param_numel)  
            ...
            assert total_available_capacity >= aligned_numel, f"Not enough buffer capacity for param {param.ds_id}"  
    ...
    def get_buffer(self, param, numel):  
        ...
        aligned_numel = self._io_aligned_numel(numel)  
        ...
        if allocated_buffer_id is None:  
            # 分配新的缓冲区  
            assert self.available_swap_in_buffers() > 0, f"No swap buffers to allocate for param {param_id} of numel = {numel}"  
            buffer_id = self.available_buffer_ids.pop()  
            self._initialize_new_buffer(buffer_id, param_id, aligned_numel)  
            allocated_buffer_id = buffer_id  
        ...
        swap_buffer = self.buffers.narrow(0, buffer_start, aligned_numel)  
        compute_buffer = swap_buffer.narrow(0, 0, numel)  
        
        self.param_id_to_swap_buffer[param_id] = swap_buffer  
        print_rank_0(f"param {param_id} is assigned shared buffer id {allocated_buffer_id} at offset {offset}")  
